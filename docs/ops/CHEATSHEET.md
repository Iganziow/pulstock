...
```bash
sudo pm2 restart pulstock-web                              # frontend
sudo systemctl reload pulstock-api   # HUP al master, sin cortar conexiones          # backend graceful
...
cd ../web && npx next build && sudo pm2 restart pulstock-web && \
echo "DEPLOY OK"
```
...
```bash
sudo systemctl restart postgresql nginx pulstock-api
sudo pm2 restart pulstock-web
sleep 10 && curl -s http://localhost:8000/api/core/health/
```
