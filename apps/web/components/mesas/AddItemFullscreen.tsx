...
/** Item devuelto al padre cuando el cajero toca "Listo".
 *  El padre (OrderPanel) los acumula en su propia lista "Pendiente"
 *  antes de hacer el POST al backend. */
...
interface AddItemFullscreenProps {
  orderId: number;
  tableName: string;
  /** Llamado al tocar "Listo". Devuelve los items seleccionados al
      padre. NO hace POST — el padre los acumula en "Pendiente" y
      confirma todo junto. */
  onConfirm: (items: PendingLineDraft[]) => void;
  /** Llamado cuando cierra sin agregar (← o cancelar). */
  onClose: () => void;
}

export function AddItemFullscreen({ orderId, tableName, onConfirm, onClose }: AddItemFullscreenProps) {
  ...
  function save() {
    // Ya no hace POST: solo devuelve los items al padre (OrderPanel),
    // que los acumula como "Pendiente" hasta que el cajero confirme
    // todo junto desde la mesa. Mario lo pidió: separar selección
    // (acá) de confirmación (en la mesa) — estilo Fudo.
    if (!cart.length) return;
    const items: PendingLineDraft[] = cart.map(c => ({
  ...
  return (
    <div style={{
      ...
      {/* Footer compacto: solo "Listo" para devolver el carrito al padre.
          La lista "Pendiente" ahora se muestra EN LA MESA (OrderPanel),
          no acá — Mario lo pidió: en este screen solo seleccionamos,
          la revisión es en la mesa. */}
      {cart.length > 0 && (
        <div style={{
          ...
          >
            ...
            {saving
              ? "Procesando…"
              : `Listo · ${cartItems} item${cartItems !== 1 ? "s" : ""} · $${fmt(cartTotal)}`}
          </button>
        </div>
      )}
